...
export const handlers = [
  ...
  http.put(
    `${BASE}/workspaces/:wsId/members/:memberId`,
  ...
  http.put(
    `${BASE}/workspaces/:wsId/accounts/:accountId`,
    async ({ params, request }) => {
  ...
  http.get(`${BASE}/workspaces/:wsId/categories`, ({ params }) =>
    HttpResponse.json(
      categories.filter((c) => c.workspaceId === params["wsId"]),
    ),
  ),

  http.post(
    `${BASE}/workspaces/:wsId/categories`,
    async ({ params, request }) => {
  ...
  http.put(
    `${BASE}/workspaces/:wsId/categories/:categoryId`,
    async ({ params, request }) => {
  ...
  http.get(`${BASE}/workspaces/:wsId/transactions`, ({ params }) =>
    HttpResponse.json(
      transactions.filter((t) => t.workspaceId === params["wsId"]),
    ),
  ),

  http.post(
    `${BASE}/workspaces/:wsId/transactions`,
    async ({ params, request }) => {
  ...
  http.put(
    `${BASE}/workspaces/:wsId/transactions/:txId`,
    async ({ params, request }) => {
  ...
  http.get(
    `${BASE}/workspaces/:wsId/reports/category-expenses`,
    ({ params }) => {
      const wsTransactions = transactions.filter(
        (t) =>
          t.workspaceId === params["wsId"] && t.transactionType === "expense",
      );
      ...
      for (const tx of wsTransactions) {
        ...
        expenseMap.set(
          tx.categoryId,
          (expenseMap.get(tx.categoryId) ?? 0) + Number(tx.amount),
        );
      }
      const wsCategories = categories.filter(
        (c) => c.workspaceId === params["wsId"],
      );
      const result = Array.from(expenseMap.entries()).map(
        ([categoryId, total]) => ({
          categoryId,
          categoryName:
            wsCategories.find((c) => c.id === categoryId)?.name ?? categoryId,
  ...
  http.get(
    `${BASE}/workspaces/:wsId/reports/account-balances`,
    ({ params }) => {
      const wsAccounts = accounts.filter(
        (a) => a.workspaceId === params["wsId"],
      );
