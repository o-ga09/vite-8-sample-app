...
Vite+とGoを使用したデモアプリケーション。
[bob](https://github.com/stephenafamo/bob) ORM を使ったマルチテナント 家計簿管理システム。
...
- **型安全フィルタ**: `SelectWhere.Transactions.TransactionType.EQ(enums.TransactionTypeExpense)` のようなコンパイル時型チェック
...
- **Repository パターン**: データアクセス層 (`internal/repository/`) を分離し、Service 層はビジネスロジックに専念
- **トランザクション管理**: Service 層で `db.WithTx()` による明示的なトランザクション境界
...
| Linter | [golangci-lint](https://golangci-lint.run/) (gofmt, govet, staticcheck 等) |
